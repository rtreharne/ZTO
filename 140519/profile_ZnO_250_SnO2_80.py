...
for i in range (0, 11):
    ax2.plot((i-5, i-5), (-3, -4), '-', color = 'green', linewidth = 3)

ax2.plot((-5,5), (-3,-3), '-', color = 'green', linewidth = 3)
ax2.plot((-5,5), (-4,-4), '-', color = 'green', linewidth = 3)

    

Z_i = []
for i in range(0, len(Z[0])):
    Z_i.append(((Z[1][i]+Z[2][i])/2))
...
for i in range (0, len(x_green)):
    ax2.text(x_green[i], y_green[i], lst[i], color = 'green')
